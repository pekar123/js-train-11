...
function customSet(arr) {
  const set = new Set(arr); // Створення множини з масиву

  // Перебираємо множину за допомогою 'for of'
  for (const value of set) {
    // Перевіряємо, чи є поточний елемент числом за допомогою оператора typeof
    if (typeof value === 'string') {
      // Якщо елемент є числом, видаляємо його з множини за допомогою метода delete
      set.delete(value);
    }
  }
...
function addElements(mySet, arr) {
  ...
  for (const element of arr) {
    mySet.add(element); // Додаємо кожний елемент масиву до множини
  }
...
function setToArray(mySet) {
    // Конвертуємо множину в масив за допомогою Array.from()
    const arr = Array.from(mySet);
  
    // Використовуємо метод filter для відфільтрування лише рядкових елементів
    const stringValues = arr.filter(item => typeof item === 'string');
    
    // Відсортовуємо рядкові елементи в алфавітному порядку
    stringValues.sort();
    
    // Повертаємо відсортований масив рядкових елементів
    return stringValues;
...
function removeDuplicatesInPlace(arr) {
  const uniqueSet = new Set(); // Створення множини для зберігання унікальних елементів
  
  for (let i = 0; i < arr.length; i++) { // Перебір елементів масиву за допомогою циклу for
    if (uniqueSet.has(arr[i])) { // Перевірка, чи елемент вже присутній у множині
      arr.splice(i, 1); // Якщо елемент вже є унікальним, видаляємо його з масиву
...
function areDisjoint(set1, set2) {
  ...
  for (const num1 of set1) {
    // Перевіряємо, чи елемент першої множини присутній у другій множині
    if (set2.has(num1)) {
      // Якщо знайдено спільний елемент, повертаємо false
      return false;
    }
  }
  // Якщо жодного спільного елемента не знайдено, повертаємо true
  return true;
  // Перебираємо першу множину за допомогою оператору for of
...
function getDifference(set1, set2) {
  // Створення порожньої множини differenceSet
  const differenceSet = new Set();

  // Перебір елементів першої множини за допомогою оператору for...of
  for (const element of set1) {
    // Перевірка, чи поточний елемент першої множини не належить другій множині
    if (!set2.has(element)) {
